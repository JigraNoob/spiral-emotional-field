...
def emit_glint(toneform):
    glint = {
        'glint.phase': 'trans',
        'glint.toneform': toneform,
        'glint.content': f'Test {toneform} glint',
        'timestamp': time.time()
    }
    response = requests.post('http://localhost:5000/glint', json=glint)
    print(f"Emitted {toneform} glint. Response: {response.status_code}")

# Emit glints to trigger fusion
emit_glint('shimmer')
time.sleep(1)
emit_glint('reverie')
