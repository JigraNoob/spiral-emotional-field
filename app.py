...
def create_app():
    ...
    try:
        ...
        app.logger.warning("No config.py found, using default configuration")
        app.config.update({
            'DEBUG': True,
            'SECRET_KEY': 'dev-key-please-change-in-production',
            'SQLALCHEMY_DATABASE_URI': 'sqlite:///spiral.db',
    ...
    from routes.memory_playback import playback_bp
    from routes.echo_tracker import echo_bp
    from routes.test_blueprint import test_bp
    from routes.echo_export import echo_export_bp
    ...
    app.logger.info(f"Registered ritual_reflection blueprint at {ritual_reflection_bp.url_prefix}")
    app.register_blueprint(ritual_invitations_bp, url_prefix='/ritual')
    app.logger.info(f"Registered ritual_invitations blueprint at {ritual_invitations_bp.url_prefix}")
    app.register_blueprint(playback_bp, url_prefix='/reflect')
    app.logger.info(f"Registered playback blueprint at {playback_bp.url_prefix}")
    app.register_blueprint(echo_bp, url_prefix='/echo')
    app.logger.info(f"Registered echo blueprint at {echo_bp.url_prefix}")
    app.register_blueprint(resonance_memory_bp, url_prefix='/resonance')
    app.logger.info(f"Registered resonance_memory blueprint at {resonance_memory_bp.url_prefix}")
    app.register_blueprint(resonance_reflect_bp, url_prefix='/resonance')
    app.logger.info(f"Registered resonance_reflect blueprint at {resonance_reflect_bp.url_prefix}")
    app.register_blueprint(get_encounters_bp, url_prefix='/get')
    app.logger.info(f"Registered get_encounters blueprint at {get_encounters_bp.url_prefix}")
    app.register_blueprint(test_bp, url_prefix='/test')
    app.logger.info(f"Registered test blueprint at {test_bp.url_prefix}")
    app.register_blueprint(echo_export_bp, url_prefix='/export')
    app.logger.info(f"Registered echo_export blueprint at {echo_export_bp.url_prefix}")
    ...
    def context_test():
        ...
        app.logger.info(f"Current app name: {current_app.name}")
        app.logger.info(f"App context: {app.app_context()}")
        return jsonify({"app_name": current_app.name})

    @app.route('/global_test')
    def global_test():
        app.logger.info("Global test route accessed!")
        return jsonify({"status": "Global test successful!"})
